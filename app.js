...
function loadSprayHistory() {
  const db = firebase.firestore();
  const userId = firebase.auth().currentUser.uid;
  
  db.collection(`users/${userId}/jobs`)
    ...
    .onSnapshot(snapshot => {
      ...
      snapshot.forEach(doc => {
        ...
        card.innerHTML = `
          <h3>${job.id}</h3>
          <p>${job.startTime.toDate().toLocaleString()}</p>
          <p>Tanks: ${job.tanks.length} | Volume: ${job.totalVolume} gal</p>
          <button class="view-job" data-id="${doc.id}">View Details</button>
        `;
...
function initMap() {
  const map = L.map('map').setView([38.0, -122.0], 15);
  ...
  const streets = L.tileLayer('https://{s}.tile.openstreetmap.org/...', {
    attribution: '© OSM'
  });
  
  // Overlays
  const vineyardBlocks = L.geoJSON(vineyardGeoJSON, {
    style: {
      color: '#ff7800',
      weight: 3,
      opacity: 0.8,
  ...
  const overlays = {
    "Vineyard Blocks": vineyardBlocks
  };
